...
public class JPAIllustrationTest extends TestCase {
	...
	public void testBasicUsage() {
		...
		inTransaction(entityManager -> {
			entityManager.persist(new EventObj("Our very first event!", now()));
			entityManager.persist(new EventObj("A follow up event", now()));
		});

		// now lets pull events from the database and list them
		inTransaction(entityManager -> {
			entityManager.createQuery("select e from Event e", EventObj.class).getResultList()
